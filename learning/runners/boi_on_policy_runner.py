...
from learning.storage.replay_buffer import FinetuneBuffer
# from learning.utils import store_code_state

import matplotlib.pyplot as plt


class BOIOnPolicyRunner:
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        if self.task_sampler_class == "OfflineSampler":
            ...
            latent_param_path=self.env.cfg.fld.load_root+"/evaluate/latent_params.pt"
            task_sampler.load_data(latent_param_path)
            print(f'[Using Latent Params from:] {latent_param_path}')
            ...
            if self.env.cfg.task_sampler.alp_gmm.random_type == "gmm":
                task_sampler.load_gmm(self.env.cfg.fld.load_root+"/evaluate/gmm.pt")
        else:
            raise Exception(f"Unknown task sampler class {self.task_sampler_class}")
        print(f"[Sampler Decided:] {self.task_sampler_class}")
        ...
        # init the finetune buffer
        self.finetune_flag=self.cfg["finetune"]
        if self.finetune_flag:
            self.finetune_buffer_size = self.env.num_envs * self.num_steps_per_env
            self.finetune_buf = FinetuneBuffer(self.env.fld_latent_channel, self.env.fld_observation_horizon, 
                                            self.env.fld_observation_dim, self.finetune_buffer_size, self.device)
            # self.env.finetune_rewards=self.finetune_rewards
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        if self.log_dir is not None and self.writer is None:
            ...
            if self.logger_type=="wandb":
                from humanoid_gym.utils.wandb_writter import WandbSummaryWriter
                
                self.writer=WandbSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.env.cfg, datetime=self.cfg["datetime"])
    ...
    def finetune(self, num_learning_iterations, init_at_random_ep_len=False):
        # initialize writer
        if self.log_dir is not None and self.writer is None:
            self.logger_type = self.cfg.get("logger", "tensorboard")
            self.logger_type = self.logger_type.lower()
        ...
        tot_iter = self.current_learning_iteration + num_learning_iterations
        self.policy_dir=os.path.join(self.log_dir, 'policy')
        self.dynamics_dir=os.path.join(self.log_dir, 'dynamics')
        for dir in [self.policy_dir, self.dynamics_dir]:
            if not os.path.exists(dir):
                os.makedirs(dir)
        self.policy_train_iteration=0
        self.update_decoder_flag=True
        for it in range(self.current_learning_iteration, tot_iter):
            start = time.time()
            # Rollout
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    # collect z to update the decoder
                    latent_t=self.env.latent_encoding.clone()
                    z_t=self.env.compute_z(latent_t)
                    ...
                    # collect the actual state to update the decoder
                    s_t=self.env.fld_state.clone()
                    self.finetune_buf.insert(s_t, z_t)
            ...
            mean_value_loss, mean_surrogate_loss = self.alg.update()
            self.policy_train_iteration+=1
            
            # if (self.policy_train_iteration%self.save_interval==0) & update_decoder_flag:
            #     mean_decoder_loss, mean_z_loss =self.env.update_decoder(self.finetune_buf)
            if self.update_decoder_flag:
                mean_decoder_loss, mean_z_loss =self.env.update_decoder(self.finetune_buf)
            # mean_decoder_loss=0
            if mean_decoder_loss<5:
                self.update_decoder_flag=False
            self.finetune_buf.clear()
            ...
            if self.log_dir is not None:
                self.log(locals())
            if it % self.save_interval == 0:
                self.save(os.path.join(self.policy_dir, "model_{}.pt".format(it)))
                self.save_fld(os.path.join(self.dynamics_dir, f"fld_finetune_model_{it}.pt"), it)
                self.log_latent_params(it)
    ...
    def log(self, locs, width=80, pad=35):
        ...
        # if self.finetune_flag & (self.policy_train_iteration%self.save_interval==0):
        #     self.writer.add_scalar("Finetune/fld_loss", locs["mean_decoder_loss"], locs["it"])
        #     self.writer.add_scalar("Finetune/z_loss", locs["mean_z_loss"], locs["it"])
        if self.finetune_flag & self.update_decoder_flag:
            self.writer.add_scalar("Finetune/fld_loss", locs["mean_decoder_loss"], locs["it"])
            self.writer.add_scalar("Finetune/z_loss", locs["mean_z_loss"], locs["it"])
    ...
    def save_fld(self, path, it):
        torch.save(
            {
                "fld_state_dict": self.env.fld.state_dict(),
                "fld_decoder_optimizer_state_dict": self.env.decoder_optimizer.state_dict(),
                "iter": it,
                }, 
            path
            )
    ...
    def load(self, path, load_optimizer=True):
        ...
        print(f'[Policy Loading From:] {path}')
