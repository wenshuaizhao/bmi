...
class FLD(nn.Module):
    ...
    def forward(self, x, k=1):
        ...
        # shapes: pred_dynamics [4,1900,27,51];latent [1900,8,51]; amplitude [1900,8];
        return pred_dynamics, latent, signal, params, z.clone().detach()
    
    def z_forward(self, x, k=1):
        ...
        # shapes: pred_dynamics [4,1900,27,51];latent [1900,8,51]; amplitude [1900,8];
        if x_length>3:
            z_shape=z.squeeze().shape
            return z.squeeze().view(*x_shape[:2],*z_shape[1:])
        else:
            return z.squeeze()
    ...
    def get_dynamics_error(self, state_transitions, k):
        ...
        with torch.no_grad():
            pred_dynamics, _, _, _, _ = self.forward(state_transitions_sequence.flatten(0, 1).swapaxes(1, 2), k)
