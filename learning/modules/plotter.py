...
import numpy as np
c_colors = np.array(["red","green","blue","yellow","pink","black","orange","purple","lime","brown","olive","cyan","sienna"])
...
class Plotter:
    ...
    def plot_pca(self, ax, manifold_collection, title=None, point_color=None, draw_line=True, draw_arrow=True):
        ...
        if point_color == None:
                if len(manifolds_pca)>10:
                    point_color = list(mcolors.CSS4_COLORS.keys())
                else:
                    point_color = list(mcolors.TABLEAU_COLORS.keys())
                point_color=c_colors

        for i, manifold in enumerate(manifolds_pca):
            if draw_line:
                ax.plot(manifold[:, 0], manifold[:, 1], color=(0.0, 0.0, 0.0), alpha=line_alpha)
            if point_color is None:
                ax.scatter(manifold[:, 0], manifold[:, 1], alpha=point_alpha, label=i, color=point_color[i])
            else:
