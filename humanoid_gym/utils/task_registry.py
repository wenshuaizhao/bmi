...
from learning.runners import BOIOnPolicyRunner
...
class TaskRegistry:
    ...
    def make_env(
        ...
        #     if args.finetune:
        #         env_cfg.env.finetune_rewards=args.finetune
    ...
    def make_alg_runner(
        ...
        if log_root is None:
            ...
            if log_root == "default":
                log_root = os.path.join(LEGGED_GYM_ROOT_DIR, "logs", train_cfg.runner.experiment_name, "policy_training")
                if name is not None and 'boi' in name:
                    log_root = os.path.join(LEGGED_GYM_ROOT_DIR, "logs", train_cfg.runner.experiment_name, "boi_policy_training")
            # log directory
            log_dir_path = os.path.join(
                log_root,
                dt + "_" + train_cfg.runner.run_name,
            )
        # create training runner
        runner_class = eval(train_cfg.runner_class_name)
        train_cfg_dict = class_to_dict(train_cfg)
        train_cfg_dict["runner"].update({'datetime':dt})
        ...
        if args.finetune is not None:
            if args.finetune:
                train_cfg.runner.resume=args.finetune
        ...
        if resume:
            ...
            print(f"[Loading policy model] from: {resume_path}")
