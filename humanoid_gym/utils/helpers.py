...
def update_cfg_from_args(env_cfg, cfg_train, args: argparse.Namespace) -> tuple:
    ...
    if env_cfg is not None:
        ...
        if args.dataset == 'normal':
            env_cfg.fld.load_root = LEGGED_GYM_ROOT_DIR+'/logs/flat_mit_humanoid/fld/dynamics_training/20240423151910'
        elif args.dataset == 'hard':
            env_cfg.fld.load_root = LEGGED_GYM_ROOT_DIR+'/logs/flat_mit_humanoid/fld/dynamics_training/20240423155422'
        elif args.dataset == 'mix':
            # env_cfg.fld.load_root = LEGGED_GYM_ROOT_DIR+'/logs/flat_mit_humanoid/fld/dynamics_training/20240429181044'
            env_cfg.fld.load_root = LEGGED_GYM_ROOT_DIR+'/logs/flat_mit_humanoid/fld/dynamics_training/20240522191815'
        if env_cfg.env.dataset is None:
            env_cfg.env.dataset = args.dataset
        if args.num_envs is not None:
            env_cfg.env.num_envs = args.num_envs
    ...
    if cfg_train is not None:
        ...
        if args.load_run is not None:
            dir_latter='/logs/flat_mit_humanoid'+args.load_run.split('/logs/flat_mit_humanoid')[-1]
            cfg_train.runner.load_run = LEGGED_GYM_ROOT_DIR + dir_latter
        if args.checkpoint is not None:
            cfg_train.runner.checkpoint = args.checkpoint
        if args.finetune is not None:
            cfg_train.runner.finetune = args.finetune
...
def get_args() -> argparse.Namespace:
    ...
    custom_parameters = [
        ...
        },
        {
            ...
            "action": "store_true",
            "default": False,
            "help": "Finetune the policy and motions",
        }
