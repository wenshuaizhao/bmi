...
class BOIHumanoid(LeggedRobot):
    ...
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.decoder_optimizer = optim.Adam(self.fld.decoder.parameters(), lr=self.cfg.fld.decoder_learning_rate, weight_decay=self.cfg.fld.decoder_weight_decay)
        
        self.decoded_obs = torch.ones(self.num_envs, self.fld_observation_dim, dtype=torch.float, device=self.device, requires_grad=False)*0.5
        ...
        self.joint_state_idx_list = []
        for state, ids in self.decoded_obs_state_idx_dict.items():
            if 'dof_pos' in state:
                self.joint_state_idx_list.extend(ids)
    ...
    def _prepare_reward_function(self):
        ...
        if self.cfg.env.finetune_rewards:
    ...
    def compute_z(self, latent_t):
        noise_level = self.cfg.domain_rand.latent_encoding_update_noise_level
        latent_param = latent_t[:, :, 1:].swapaxes(1, 2).flatten(1, 2)
        latent_param += torch.randn_like(latent_param, device=self.device, dtype=torch.float, requires_grad=False) * self.latent_param_std * noise_level
        latent_t[:, :, 1:] = latent_param.view(self.num_envs, 3, self.fld_latent_channel).swapaxes(1, 2)
        phase = latent_t[:, :, 0]
        frequency = latent_t[:, :, 1]
        amplitude = latent_t[:, :, 2]
        offset = latent_t[:, :, 3]
        reconstructed_z = amplitude.unsqueeze(-1) * torch.sin(2 * torch.pi * (frequency.unsqueeze(-1) * self.fld.args + phase.unsqueeze(-1))) + offset.unsqueeze(-1)
        
        return reconstructed_z
    
    def update_decoder(self, finetune_buffer):
        decoder_loss, z_loss_value = 0, 0
        self.fld.decoder.train()
        
        for it in range(1):
            decoder_data_generator = finetune_buffer.feed_forward_generator(
                self.cfg.fld.decoder_num_mini_batches,
                finetune_buffer.num_samples // self.cfg.fld.decoder_num_mini_batches
            )
            for (state_target, z) in decoder_data_generator:
                decoded_obs_buf_pred = self.fld.decoder(z.detach())
                encoded_z=self.fld.z_forward(decoded_obs_buf_pred)
                decoded_obs_buf_raw = decoded_obs_buf_pred.swapaxes(1, 2)
                decoded_obs = decoded_obs_buf_raw[:, -1, :] * self.state_transitions_std + self.state_transitions_mean
                
                # reconstruction loss
                loss=0
                loss=self.compute_decoder_loss(decoded_obs, state_target)
                z_loss=self.compute_z_loss(encoded_z, z.detach())
                z_loss_value += z_loss.item()
                decoder_loss += loss.item()
                self.decoder_optimizer.zero_grad()
                (loss+200*z_loss).backward()
                self.decoder_optimizer.step()
        self.fld.decoder.eval()
        mean_decoder_loss=decoder_loss/self.cfg.fld.decoder_num_mini_batches
        mean_z_loss=z_loss_value/self.cfg.fld.decoder_num_mini_batches
        return mean_decoder_loss, mean_z_loss
    
    def compute_decoder_loss(self, input, target):
        return torch.mean(torch.sum(torch.square((input - target)), dim=-1))
