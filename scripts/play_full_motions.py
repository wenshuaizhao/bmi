...
def play(args: argparse.Namespace):
    ...
    datasets_root = os.path.join(LEGGED_GYM_ROOT_DIR + "/resources/robots/mit_humanoid/datasets/mix")
    ...
    if RECORD_FRAMES:
        export_root=os.path.join(LEGGED_GYM_ROOT_DIR, f'logs/videos/play_fld/{datetime.now().strftime("%Y%m%d%H%M%S")}_full_motions')
        if not os.path.exists(export_root):
            os.makedirs(export_root)
        print(f'[Video Folder:] {export_root}')
    ...
    for i in range(10 * int(env.max_episode_length)):
        ...
        if RECORD_FRAMES:
            filename = os.path.join(export_root, f"{i}.png")
            # print(filename)
        ...
        if i < stop_state_log:
            ...
            # robot_states=env.rigid_body_pos
            
            logger.log_states(
                {   
                    "right_foot_hight":env.rigid_body_pos[robot_index,env.feet_indices[-1],2].cpu().numpy(),
                    "left_foot_hight":env.rigid_body_pos[robot_index,env.feet_indices[0],2].cpu().numpy(),
