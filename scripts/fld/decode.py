...
class FLDEvaluate:
    ...
    def __init__(self, state_idx_dict, observation_horizon, num_consecutives, device, args=None):
        ...
        self.decoded_root = os.path.join(self.load_run, "decoded")
        if not os.path.exists(self.decoded_root):
            os.makedirs(self.decoded_root)
        ...
        print(f'[Modal Loading:] We load trained model from {os.path.join(self.load_run, model)}')
    ...
    def prepare_data(self):
        ...
        csv_file=os.path.join(self.decoded_root, 'dynamics_error.csv')
        dynamics_error={}

        for i, motion_name in enumerate(motion_name_set):
            motion_path = os.path.join(datasets_root, "motion_data_" + motion_name + ".pt")
            self.ori_motion_data=torch.load(motion_path, map_location=self.device)
            motion_data = self.ori_motion_data[:, :, self.dim_of_interest]
            ...
            # compute latent dynamics error. motion_data shape:[10,240,27]
            state_transitions = (motion_data - self.state_transitions_mean) / self.state_transitions_std
            error = self.fld.get_dynamics_error(state_transitions, k=4).mean().item() # what is the meaning of k? number of consecutives
            print(f"[Motion Loader] Motion {motion_name} dynamics error: {error}")
            dynamics_error.update({motion_name:error})
            
        with open(csv_file, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile,quoting=csv.QUOTE_ALL)
            writer.writerow(['Date:', datetime.now().strftime("%Y%m%d%H%M%S")])
            for key, value in dynamics_error.items():
                writer.writerow([key, value])
            csvfile.write("\n")
        
        motion_data_collection = torch.cat(motion_data_collection, dim=0)
        self.motion_data_collection = motion_data_collection.clone()
        # motion_data_collection shape [4, 10, 240, 27]
        ...
        # motion_data_collection = motion_data_collection.unfold(2, self.observation_horizon, 1).swapaxes(-2, -1)
    ...
    def fit_gmm(self):
        ...
        with torch.no_grad():
            ...
            torch.save(
                latent_params,
                self.decoded_root + f"/latent_params.pt"
                )
            torch.save(
                {
                    "gmm_state_dict": gmm.state_dict(),
                    },
                self.decoded_root + f"/gmm.pt"
                )

    def sample_latent(self):
        with torch.no_grad():
            eval_traj = self.state_transitions_data[0, 0, :, :self.observation_horizon, :].swapaxes(1, 2)
            pred_dynamics, latent, signal, params, _ = self.fld(eval_traj) # pred_dynamics is predicted by local latent parameters of frames
            ...
            decoded_traj_pred = self.fld.decoder(latent_sample_z) # this decoded_traj_pred is different from pred_dynamics. Because it is predicted by rollouted (auto-regressive) latent parameters.
            ...
            #NOTE replace the decoded_traj with the ground truth trajectory from the dataset to validate the accumulated base pos and orientation.
            decoded_traj=self.motion_data_collection[0,0].unsqueeze(0)
        ...
        decoded_path = os.path.join(self.decoded_root, "motion_data_decoded_gt.pt")
        ...
        # decoded_base_ang_vel = self.motion_data_collection[0,0,49:].unsqueeze(0)[:, :, torch.tensor(state_idx_dict["base_ang_vel"]) - self.observation_start_dim]
        ...
        # decoded_base_lin_vel = self.motion_data_collection[0,0,49:].unsqueeze(0)[:, :, torch.tensor(state_idx_dict["base_lin_vel"]) - self.observation_start_dim]
    ...
    def save_figure(self):
        self.fig_dir=os.path.join(self.decoded_root, "figs")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_dir", type=str, default='20240423151910', help="run folder where we want to load the model, 20240423155422 or 20240423151910")
