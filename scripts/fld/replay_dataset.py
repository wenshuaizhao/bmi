...
if RECORD_FRAMES:
    export_root=os.path.join(LEGGED_GYM_ROOT_DIR, f'logs/videos/original_data/{datetime.now().strftime("%Y%m%d%H%M%S")}')
    ...
    for dataset_idx in range(aggregated_dataset_collection.shape[0]):
        for traj_idx in range(aggregated_dataset_collection.shape[1]):
            ...
            for step_idx in range(aggregated_dataset_collection.shape[2]):
                ...
                if RECORD_FRAMES:
                    filename = os.path.join(export_root, f"{img_idx}.png")
