...
class FLDEvaluate:
    ...
    def evaluate(self):
        ...
        self.dataset_pca_collection=[]
        self.plot_dataset_pca()
        # self.fit_gmm()
        # self.sample_latent()
        
    def plot_dataset_pca(self):
        for i in range(self.num_motions):
                eval_traj = self.state_transitions_data[i, 0, :, :self.observation_horizon, :].swapaxes(1, 2)
                self.dataset_pca_collection.append(eval_traj[:,:,0])
        self.plotter.plot_pca(self.ax3, self.dataset_pca_collection, )
    ...
    def save_dataset_pca(self):
        self.fig_dir=os.path.join('/home/username/Downloads/Research/CoRL2024', "figs_to_check")
        if not os.path.exists(self.fig_dir):
            os.makedirs(self.fig_dir)
        for i in plt.get_fignums():
            plt.figure(i)
            plt.savefig(f'{self.fig_dir}/Figure_pca_{i}.pdf', bbox_inches='tight')
