...
class FLDEvaluate:
    ...
    def __init__(self, state_idx_dict, observation_horizon, num_consecutives, device, args=None):
        ...
        self.fig1, self.ax1 = plt.subplots(4, 1,layout="constrained")
    ...
    def prepare_data(self):
        ...
        self.motion_names=motion_name_set
    ...
    def evaluate(self):
        ...
        self.plot_reconstructed_motions()
        # self.sample_latent()
    
    def plot_reconstructed_motions(self):
        title="Motion Reconstruction (Flattened)" + " " + str(1) + "x" + str(self.fld.input_channel*self.observation_horizon)
        plot_traj_index = 0
        with torch.no_grad():
            ...
            for i in range(4):
                ...
                self.plotter.plot_curves(self.ax1[i], torch.vstack((pred[plot_traj_index].flatten(0, 1), eval_traj[plot_traj_index].flatten(0, 1))), -1.0, 1.0, -5.0, 5.0, title=title, show_axes=False)
                self.ax1[i].text(-0.1, -4.0, motion_name.title().replace('_', ' '))
                if i==0:
                    self.ax1[i].legend(['Reconstructed', 'Target'])

    def plot_latent_phase(self):
        title="Learned Phase Timing"  + " " + str(latent_dim) + "x" + str(2)
        with torch.no_grad():
            # for i in range(self.num_motions):
            for i in range(4):
                if i!=0:
                ...
                plot_traj_index = 0
                self.plotter.plot_circles(self.ax1[i], latent_sample_phase[plot_traj_index], latent_sample_amplitude[plot_traj_index], title=title, show_axes=False)
        # self.fig1.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)
    ...
    def save_latent_phase(self):
        self.fig_dir=os.path.join('/home/username/Downloads/Research/CoRL2024', "figs")
        if not os.path.exists(self.fig_dir):
            os.makedirs(self.fig_dir)
        for i in plt.get_fignums():
            plt.figure(i)
            plt.savefig(f'{self.fig_dir}/latent_phase_Figure_{i}.pdf', bbox_inches='tight')
    
    def save_reconstructed_motions(self):
        self.fig_dir=os.path.join('/home/username/Downloads/Research/CoRL2024', "figs")
        if not os.path.exists(self.fig_dir):
            os.makedirs(self.fig_dir)
        for i in plt.get_fignums():
            plt.figure(i)
            plt.savefig(f'{self.fig_dir}/recontructed_motions_scae_{i}.pdf', bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    fld_evaluate.save_reconstructed_motions()
    # plt.show()
